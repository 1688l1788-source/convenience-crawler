...
def classify_category_gs25(title):
    ...
    # 1. 생활용품 (가장 먼저 제외)
    if any(k in title for k in ['물티슈', '티슈', '마스크', '생리대', '중형', '대형', '오버나이트', '입는오버', '세제', '건전지', '스타킹', '샴푸', '린스', '치약', '칫솔', '면도기', '가글', '가그린', '패드', '클렌징', '비누', '로션', '핸드크림', '밴드']):
        return "생활용품"
    # 2. 식사/면류/간편식
    if any(k in title for k in ['육개장', '국밥', '떡볶이', '수제비', '찌개', '탕', '국', '닭가슴살', '도시락', '김밥', '주먹밥', '샌드위치', '햄버거', '햇반', '라면', '우동', '짬뽕', '파스타', '치킨', '핫바', '소시지', '만두']):
        return "식사"
    # 3. 과자/간식/디저트
    if any(k in title for k in ['킷캣', '캔디', '젤리', '초코', '초콜릿', '쿠키', '껌', '스낵', '칩', '봉지', '프레첼', '아몬드', '땅콩', '찹쌀떡', '케익', '양갱', '파이', '강정', '카라멜', '빼빼로', '육포', '돌자반', '맛밤']):
        return "과자"
    # 4. 아이스크림
