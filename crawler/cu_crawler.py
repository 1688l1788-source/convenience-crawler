...
def classify_category_gs25(title):
    """GS25 상품명을 분석하여 카테고리 반환 (우선순위 기반 정밀 분류)"""
    
    # 1. 생활용품 (최우선 순위: 브랜드 및 위생/뷰티/화학 키워드)
    ...
    # 2. 식사/면류/간편식 (요청 키워드 반영)
    if any(k in title for k in [
        '그래놀라', '통곡물밥', '크랩', '닭껍질튀김', # 👈 추가된 요청 키워드
    ...
    # 3. 과자/간식/디저트 (요청 키워드 반영)
    if any(k in title for k in [
        '말차빵', '허쉬다크', '그릭요거트', # 👈 추가된 요청 키워드
        '킷캣', '캔디', '젤리', '초코', '초콜릿', '쿠키', '껌', '스낵', '칩', '봉지', 
        '프레첼', '아몬드', '땅콩', '찹쌀떡', '케익', '양갱', '파이', '강정', '카라멜', 
        '빼빼로', '육포', '돌자반', '맛밤', '카스텔라', '푸딩', '오팜'
    ]):
        return "과자"
...
def crawl_cu(supabase):
    print("\n🚀 CU 크롤링 시작...")
    categories = {"GD_01": "과자", "GD_02": "음료", "GD_03": "식사", "GD_04": "아이스크림", "GD_05": "생활용품"}
...
def main():
    ...
    except Exception as e: print(f"❌ CU 에러: {e}")
    try: crawl_gs25(supabase)
    except Exception as e: print(f"❌ GS25 에러: {e}")
    print("\n🎉 크롤링 완료!")
