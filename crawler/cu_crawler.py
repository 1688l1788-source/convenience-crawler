...
def get_standard_category(title, raw_category=None):
    ...
    # 6. 원본 카테고리 매핑
    if raw_category:
        if raw_category in ["간편식사", "식품"]: return "식사/라면" # 즉석조리는 제외
...
def parse_cu_product(item, raw_cat_name):
    try:
        ...
        # 🚫 [필터링] 즉석조리 및 GET커피 강제 제외
        # 카테고리가 '즉석조리'이거나 제목에 'GET'커피 관련 내용이 있으면 스킵
        if raw_cat_name == "즉석조리": return None
        if "GET" in title and ("아메리카노" in title or "라떼" in title or "커피" in title): 
            return None
        if "델라페" in title and "얼음" in title: # 얼음컵 등 제외 희망 시
            pass
...
def crawl_cu(supabase):
    ...
    # 1. [중요] 기존 CU 데이터 완전 삭제 (오염된 데이터 정리)
    supabase.table("new_products").delete().eq("brand_id", 1).execute()
    # 2. 혹시 몰라 '즉석조리'라는 이름으로 들어간게 있다면 추가 삭제
    supabase.table("new_products").delete().eq("original_category", "즉석조리").execute()
