...
def parse_cu_product(item, raw_cat_name):
    try:
        ...
        # (A) NEW 이미지 확인 (tag_new.png)
        # item 내부의 모든 이미지 태그를 검사
        ...
        return {
            ...
            "is_active": True,
            "external_id": gdIdx,
            "is_new": is_new # tag_new.png 발견 시 True
        }
    except: return None
...
def crawl_cu(supabase):
    ...
    # 데이터 초기화 (증분 업데이트 원하면 삭제하지 말고 upsert만 유지)
    # 전체 갱신을 위해 삭제함
