...
def get_standard_category(title, raw_category=None):
    ...
    # 2. [식사/라면] (식품으로 오인될 수 있는 '바' 정규식 처리 포함)
    ...
    if any(k in title for k in [
        ...
        '크런키', '에너지바', '단백질바', '프로틴바', '자유시간' # 바 형태지만 과자인 것들
    ]):
        return "과자/간식"

    # 4. [아이스] (위에서 식품 '바'를 걸러냈으므로 나머지는 아이스크림)
